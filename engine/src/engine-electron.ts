...
document.addEventListener('click', (e: MouseEvent) => {
  ...
  // external links — Electron's will-navigate handler opens these in system browser
...
(async () => {
  ...
  window.newweb = {
    ...
    auth:     (_success, _passwordHash, _username) => { /* hookup point — electron auth handling TBD */ },
    apiFetch: (_method, _url, _body) => { /* hookup point */ },
  };

  document.getElementById('nw-view-data')!.addEventListener('click', showDataModal);

  const initial = location.hash.length > 1 ? location.hash.slice(1) : null;
  if (initial) await replacePage(initial);
})().catch(err => {
